...
public class UpdateById implements ObjectCommand, Serializable {
    ...
    public ResultDTO validate(String[] args) {
        if(args != null && args.length == 1) {
            StringDTO idValidation = validateInt(args[0], CommandArgument.ID.toString(), Route.MIN_ID, Integer.MAX_VALUE);
        ...
        try {
            for (int k : collectionMap.keySet())
                if (collectionMap.get(k).getId() == id) {
                    ...
                    return new StringDTO(true, "Объект успешно изменён");
                }
    ...
    public CommandArgument[] getArgs(){
        return new CommandArgument[]{CommandArgument.ID};
    }
}