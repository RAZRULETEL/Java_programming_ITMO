...
public class Main {
    ...
    public static void main(String[] args){
        ...
        if(System.getenv().containsKey("PORT"))
            ...
            System.out.println("ѕеременна€ окружени€ с портом не обнаружена");
        if(System.getenv().containsKey("FILE_NAME"))
            filename = System.getenv("FILE_NAME");
        else
            System.out.println("ѕеременна€ окружени€ не обнаружена");
            ...
            if (consoleReader.ready()){
                mainProcessor.processLine(consoleReader.readLine())
                        .ifPresentOrElse(value -> System.out.println(mainProcessor.processCommand(value))
                                , () -> {System.out.println("ѕохоже, что данной команды не существует, доступны следующие команды: "+mainProcessor.getServerCommands());});
            }
