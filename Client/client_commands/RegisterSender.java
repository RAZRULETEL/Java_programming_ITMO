...
import Shared.commands.Register;
...
public class RegisterSender implements LocalCommand, LocalizedCommand {
    ...
    public ResultDTO execute(AbstractRouteCollection collection) {
        Register regPack = new Register();
        if(!regPack.validate(new String[]{login, password}).getSuccess())
            return regPack.validate(new String[]{login, password});
        AuthPacket pack = new AuthPacket(null, null, regPack);
        ...
        if(answer != null && answer.isPresent() && answer.get().getSuccess()) {
            ...
            return answer.get() instanceof StringDTO ? answer.get() : new StringDTO(true, "Регистрация успешна");
        }
        return answer != null && answer.isPresent() && answer.get() instanceof StringDTO ? answer.get() : new StringDTO(false, "Ошибка регистрации");
    }
    ...
    public CommandMessage validateLocaled(String[] args) {
        ...
        if(!logVal.getSuccess())
            return CommandMessage.InvalidArgument.setArgument(CommandArgument.LOGIN);

        StringDTO passVal = validateString(args[1], CommandArgument.PASSWORD.toString());
        if(!passVal.getSuccess())
            return CommandMessage.InvalidArgument.setArgument(CommandArgument.PASSWORD);
    ...
    public CommandMessage executeLocaled(AbstractRouteCollection collection) {
        ...
        if(res.getSuccess())
            return CommandMessage.RegistrationSuccess;
