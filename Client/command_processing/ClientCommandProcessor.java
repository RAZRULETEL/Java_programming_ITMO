...
public class ClientCommandProcessor extends CommandProcessor {
    ...
    public ClientCommandProcessor(ListResourceBundle currentLocale){
    ...
    public ResultDTO processCommand(Command command) {
        ...
        if(command instanceof LocalCommand){
            ...
            if (NetworkTools.getInstance().sendObject(new AuthPacket(login, pass, command))) {
                Optional<ResultDTO> result;
                result = NetworkTools.getInstance().receiveNextResultPacket();
                return result.orElseGet(() -> new StringDTO(false, "Ошибка получения ответа"));
            }else
                return new StringDTO(false, "отправка команды прервана");
        }
    }

    public CommandMessage processLocalizedLocalCommand(LocalizedCommand command){
        if(command == null) {
            return null;
        }
        if(command instanceof LocalCommand){
            return command.executeLocaled(null);
        }else {
            throw new IllegalArgumentException("Only local commands supported");
        }
    }
